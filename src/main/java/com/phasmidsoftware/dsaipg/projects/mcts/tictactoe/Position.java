...
public class Position {
    ...
    public Position move(int player, int x, int y) {
        ...
        if (matrix[x][y] < 0) {
            // TO BE IMPLEMENTED
            matrix[x][y] = player; // SOLUTION
//            return new Position(matrix, count, player);
    ...
    public List<int[]> moves(int player) {
        ...
        for (int i = 0; i < gridSize; i++)
            for (int j = 0; j < gridSize; j++)
                if (grid[i][j] < 0) {
                    result.add(new int[] {i, j}); // SOLUTION
                }
                    // TO BE IMPLEMENTED
         ;
        // END SOLUTION
    ...
    boolean threeInARow() {
        // TO BE IMPLEMENTED
        for(int i = 0; i < gridSize; i++) { // SOLUTION
    ...
    /**
     * Method to determine if there are three in a row for any player.
     * @return Optional.of(winningPlayer) if there's a win, Optional.empty() otherwise
     */
//    Optional<Integer> checkThreeInARow() {
//        // Check for player 0
//        int[] player0 = new int[]{0, 0, 0};
//        // Check for player 1
//        int[] player1 = new int[]{1, 1, 1};
//
//        // Check rows and columns
//        for(int i = 0; i < gridSize; i++) {
//            int[] rowProj = projectRow(i);
//            int[] colProj = projectCol(i);
//
//            if(Arrays.equals(rowProj, player0)) return Optional.of(0);
//            if(Arrays.equals(rowProj, player1)) return Optional.of(1);
//            if(Arrays.equals(colProj, player0)) return Optional.of(0);
//            if(Arrays.equals(colProj, player1)) return Optional.of(1);
//        }
//
//        // Check diagonals
//        int[] diagMain = projectDiag(true);
//        int[] diagAlt = projectDiag(false);
//
//        if(Arrays.equals(diagMain, player0)) return Optional.of(0);
//        if(Arrays.equals(diagMain, player1)) return Optional.of(1);
//        if(Arrays.equals(diagAlt, player0)) return Optional.of(0);
//        if(Arrays.equals(diagAlt, player1)) return Optional.of(1);
//
//        return Optional.empty();
//    }
//
//    // Keep original threeInARow for backward compatibility
//    boolean threeInARow() {
//        return checkThreeInARow().isPresent();
//    }
//
//    // Modified winner method that uses the enhanced check
//    public Optional<Integer> winner() {
//        if (count <= 4) return Optional.empty(); // Not enough moves for a win
//        return checkThreeInARow();
//    }
